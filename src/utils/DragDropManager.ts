export class DragDropManager {
    ...
    private handleMouseDown(event: MouseEvent): void {
        ...
        // 检查是否点击了交互元素（按钮、输入框等），这些元素不应该触发拖拽
    ...
    private handleMouseUp(_event: MouseEvent): void {
        ...
        if (this.isDragging && this.placeholder) {
            ...
            if (this.callbacks.onDragEnd && this.dragStartIndex !== -1) {
                ...
                const adjustedFinalIndex = Math.max(0, Math.min(targetRealIndex, maxIndex));

                if (this.dragStartIndex !== adjustedFinalIndex) {
                    this.callbacks.onDragEnd(this.dragStartIndex, adjustedFinalIndex);
                }
            }
        }
