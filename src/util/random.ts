...
export class MersenneTwister {
    ...
    public constructor(seed: number = Date.now() ^ Math.floor(Math.random() * 0xFFFFFFFF)) {
        this.N = 624;
        this.M = 397;
...
export class xorShiftRandom {
    ...
    public constructor(seed = Date.now() ^ Math.floor(Math.random() * 0xFFFFFFFF)) {
        this.seed = seed;
    }
 ...
 * @param count Maximum value (exclusive)
 * @param start
 * @throws {RangeError} If count is less than one or greater than max
 * */
export function generateUniqueRandomNumbers(count: number, start: number = 0): number[] {
    if (!Number.isInteger(count)) {
        throw new TypeError('Parameters must be integers');
    }
    if (count < 0) throw new RangeError('Max must be non-negative');
    if (count < 1) throw new RangeError('Count must be larger than 1');

    const allNumbers = Array.from({length: count}, (_, i) => i + start);
    return shuffleArray(allNumbers);
}

export function randomCover(): string {
    return `/img/audio/cover/audio-${randInt(1, __INNER_COVER_COUNT__)}.webp`;
}